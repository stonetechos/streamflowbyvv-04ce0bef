...
**Memory Engine** — explicit, opt-in preference storage (§8).

**Prompt Library** — versioned prompt assets (§7).

**LLM Adapter** — provider abstraction (§6).

**ComplianceService** — existing Foundation authority, consulted, never re-implemented (§9).
...
Po never guesses a required slot. When a slot is missing or an entity resolves ambiguously (two friends named similarly, a title matching several works, an unspecified provider), the Conversation Manager suspends the plan and asks one short question offering concrete options — the example in the brief ("choose from your preferred services, or specify Netflix, Prime Video, YouTube, another provider?") is the canonical pattern.
