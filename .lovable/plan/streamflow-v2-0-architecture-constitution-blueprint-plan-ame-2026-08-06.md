...
1. **Experience (UX) Engine** — a cross-cutting presentation-support subsystem, not one of the 13 Domain Engines. The 13 Domain Engines own business capabilities; the Experience Engine owns motion, animation, accessibility, onboarding, loading states, empty states, delight, and visual consistency. It owns **no** business state, playback authority, room authority, synchronization authority, permissions, or domain decisions. Documented as its own chapter plus a clearly-labelled section in the engine pack; it maps onto the existing design system, localization, and accessibility foundations.
...
3. **Performance Budget** — every metric records three distinct values: **Provisional Target** (design-time intent, explicitly non-binding), **Measured Baseline** (what the system actually does today, with date and method), and **Certified Threshold** (the pass/fail bar enforced at the release gate). Provisional targets are never presented as production commitments. Metrics covered: invite-to-join latency, voice connection latency, ready propagation latency, countdown spread, reconnect recovery, cold launch, warm launch, memory, CPU, battery impact, network utilization — each with measurement method and the certification profiles it is measured under.
